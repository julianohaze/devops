class tomcat::server($connectors = [], $data_sources = []) {
	...
	file { "/etc/default/tomcat7":
		...
		notify  => Service["tomcat7"], #Envia comando para reiniciar o serviço do tomcat7 quando o arquivo tomcat7 for alterado.
	}
   ...
   file { "/var/lib/tomcat7/conf/server.xml":
	...
	service { "tomcat7": #Definição do serviço do tomcat7 
	  ensure     => running,
	  enable     => true,
	  hasstatus  => true,
	  hasrestart => true,
